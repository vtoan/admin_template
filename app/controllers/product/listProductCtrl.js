angular.module("app").controller("listProductCtrl", [
	...
	function (
		...
		$scope.onSearch = function (query) {
			$scope.products = dataHolder.filter(
				(p) =>
		...
		$scope.onRemoveProm = function (product) {
			UIPopup(`Xoá khuyến mãi SP này ?`, "Đồng ý", "Không", () =>
				removeProm(product)
			);
		};
		...
		$scope.onShowDetail = function (id) {
			$cookies.put(
				"product_" + id,
				JSON.stringify(dataHolder.find((p) => p.id == id))
			);
		...
		function initData() {
			$q.all([
				...
				promService
					.reqPromProduct()
					.then((response) => addPromotion(response)),
			])
		...
		function addPromotion(response) {
			...
			response.forEach((prom) => {
				...
				let prodIDs = prom.productIds.split(",");
				//
				if (cateId) {
