angular.module("app").controller("productDetailCtrl", [
	...
	function (
		...
		//toaset
		let toaster = {};
		let validNotify = helper.notifyStatus(
			"Chưa nhập các thông tin sản phẩm bắt buộc"
		);
		// ============== property  ===============
		//state
				...
				UIPopup(
					...
					function () {
						if (isCreate)
							callService(
								productService
									.reqAdd($scope.prod, $scope.srcImages)
									.then(() => {
										toaster.show();
		...
		$scope.onRemoveImage = function (e, imgName) {
			UIPopup("Xoá hình ảnh này", "Đồng ý", "Không", function () {
				...
				let imgObj = $scope.srcImages.find(
		...
		function initData() {
			...
			// get product exsist;
				...
				productService
					...
					.then(function (response) {
						...
						$scope.srcImages = helper.imageToObject(
							response.images
						);
					})
			...
			$q.all([
				cateService
					.reqLists()
					.then((response) => ($scope.categorys = response)),
				wireService
					.reqLists()
					.then((response) => ($scope.wires = response)),
				bandService
					.reqLists()
					.then((response) => ($scope.bands = response)),
			])
				.catch((ex) => $scope.ex.setError(ex))
				.finally(() => ($scope.isLoading = false));
		}
		...
		function getDataStored(stringKey, id) {
			if (!id) {
				$scope.ex.setError("Không tìm thấy dữ liệu, [id is empty]");
				return;
			}
			...
			if (data != null) {
				...
				$scope.ex.setError("Không tìm thấy dữ liệu, [data is empty]");
			return data;
		}
		...
		function callService(obj, msg, itemId) {
			...
			$scope.isLoading = true;
			$scope.$apply();
			obj.catch((erCode) => toaster.show(false, erCode)).finally(
				() => ($scope.isLoading = false)
			);
		}
		//================= exec ===================
