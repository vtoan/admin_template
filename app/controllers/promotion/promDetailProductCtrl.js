angular.module("app").controller("promDetailProductCtrl", [
	...
	function (
		...
		$scope.onSave = function () {
			...
			if (!$scope.prom.name || !$scope.prom.discount)
				...
				UIPopup(
					...
					function () {
						...
						if ($scope.isCreate)
							...
							callService(
								promService
									.reqUpdatePromProduct(dataHolder, [
										$scope.prom,
									])
									.then((response) => {
										toaster.show(response);
		...
		function initData() {
			...
			$q.all([
				cateService
					.reqLists()
					.then((response) => ($scope.categories = response)),
				bandService
					.reqLists()
		...
		function parseDiscountVal() {
			...
			if (!valDiscount.endsWith("%"))
				return Number(valDiscount) ? true : false;
			//
