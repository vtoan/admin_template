angular.module("app").controller("orderDetailCtrl", [
	...
	function (
		...
		$scope.onChangeStatus = function (status) {
			UIPopup(
				`Thay đổi ' ${$scope.statusOrder[status]} ' đơn hàng này ?`,
				"Đồng ý",
				"Không",
		...
		$scope.findProduct = function (id) {
			...
			if (idx == -1) {
				$scope.ex.setError(
					"Không tìm thấy sản phẩm phù hợp với giỏ hàng"
				);
				return;
			}
		...
		$scope.parseToCost = function (val) {
			...
			return !parseInt(val)
				? val * 100 + " %"
				: $filter("currency")(val, "", 0) + " đ";
		};

		$scope.getAmountPay = function () {
			let pay = calPriceDiscount(
				$scope.totalAmount,
				$scope.order.promotion
			);
			$scope.fees.forEach((item) => {
				let cost = item.cost;
				pay = parseInt(cost) == cost ? pay + cost : pay / cost;
			});
			return pay;
		};
		//================= private ===================
		function initData() {
			...
			orderService
				...
				.then((response) => {
					...
					let stringIDs = response.map((od) => od.id);
					getProductInCart(stringIDs.toString());
				})
		...
		function getProductInCart(stringIDs) {
			productService
				.reqItems(stringIDs)
				.then((response) => ($scope.productOrders = response))
				.catch((ex) => $scope.ex.setError(ex));
		}
		...
		function getDataStored(stringKey, id) {
			if (!id) {
				$scope.ex.setError("Id dữ liệu trống");
				return;
			}
			...
			if (data != null) {
				$cookies.remove(key);
			} else $scope.ex.setError("Không tìm thấy dữ liệu");
			return data;
		}
